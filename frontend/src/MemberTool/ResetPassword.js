...
function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await fetch(
      'http://127.0.0.1/Allend/backend/public/api/reset-password',
      {
        method: 'POST',
        body: formDataToSend,
      }
  ...
  return (
    <div style={formContainerStyle}>
      <Form onSubmit={handleSubmit}>
        ...
        <div
          className='text-center mb-3 rounded-5'
          onClick={handleHiddenPassword}
          style={{ backgroundColor: 'blanchedalmond', cursor: 'pointer' }}
        >
          {showPWD ? (
            <>
              隱藏密碼
