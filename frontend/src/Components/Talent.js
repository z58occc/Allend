...
function Talent() {
    ...
    const fetchtalent = async () => {
      try {
        const respone = await axios.get(
          `http://localhost/Allend/backend/public/api/talent?mid=${mid}`
        );
  ...
  const renderEstablishedCase = (item) => (
    <div style={{}}>
      <div id='price'>
        <span
          style={{ borderRadius: '10px', padding: '2px', border: 'solid 1px' }}
        >
          案件
        </span>
      ...
      <div>
        <Row>
          <Col xs lg='1'>
            <Image
              src={item.d_avatar === '' ? member : item.d_avatar}
              roundedCircle
              width='50'
              height='50'
          ...
          <Col xs lg='10'>
            ...
            >
              發案人評價：
              {Array.from({ length: item.demmand_star }, (_, i) => (
                <CiStar key={i} />
              ))}
              <br></br>
        ...
        <Row className='mt-1' style={{ marginLeft: '50px' }}>
          <Col xs lg='1'>
            <Image
              src={item.s_avatar === '' ? member : item.s_avatar}
              roundedCircle
              width='50'
              height='50'
              style={{ cursor: 'pointer' }}
      ...
      <div className='container'>
        ...
        <div className='row mt-5 p-5'>
          ...
          <div className='col-sm-9'>
            ...
            <hr></hr>
            <div style={{ fontSize: '1.2rem', padding: '0.5rem 0 1rem 0' }}>
              關於我
            </div>
            <div
              id='about'
              style={{ width: 500, height: 250, fontWeight: '600' }}
            >
            ...
            <div
              id='item'
              style={{ fontSize: '1.2rem', padding: '0.5rem 0 1rem 0' }}
            >
              作品
            </div>
            <div className='row '>
            ...
            <div className='row'>
              {talent.video &&
                talent.video.map((item, index) => (
                  <div className='col-sm-4'>
                    <div className='card'>
                      <div className='card-header'>
                        <YouTubeEmbed url={item.src} />
                      </div>
                      <div className='card-body'>{item.v_name}</div>
                    </div>
                  </div>
                ))}
            </div>
            ...
            <div className='row'>
              {talent.service &&
                talent.service.map((item, index) => (
                  <Link
                    to={`/serve/${mid}/${item.sid}`}
            ...
            <div className='row'>
              {talent.established_case &&
                talent.established_case
                  .slice(0, 3)
                  .map((item, index) => renderEstablishedCase(item))}
              {talent.established_case &&
                talent.established_case.length > 3 && (
                  <div onClick={handleExpand}>{expanded ? '收起' : '展開'}</div>
                )}
              {expanded &&
                talent.established_case &&
                talent.established_case
            ...
            >
              {talent.case_member &&
                talent.case_member.map((item, index) => (
