...
function Findcase() {
  ...
  useEffect(() => {
    ...
    function fetchData({ type = "", city = [], budget = [] }) {
      console.log(city);
      //類別
      switch (type) {
        case "1":
      ...
      if (city == false) {
        ...
        setPosts((preData) => {
          return (
            preData.filter(item => city.includes(item.country_city))
          )
        })
      ...
      //     return (
      //       preData.filter(item => {
      //         return budget.some(range => item.d_amount >= range.min && item.d_amount <= range.max)
      //       })
      //     )
      //   })
    ...
    fetchData({ type: type, city: countryQuery, budget: budgetQuery });

  }

    , [type, checkedState, budgetstate])
