...
const LeftVerticalNavbar = () => {
  const { isGoogle, infoCompleted, emailVerified } =
  ...
  return (
    <Navbar bg='light' variant='light' expand='lg' className='flex-column'>
      ...
      >
        <Image
          src={usermember.image === '' ? member : usermember.image}
          roundedCircle
          width='100'
          height='100'
      ...
      <Link
        to={`/talent/${usermember.mid}`}
        style={{ fontSize: '20px', fontWeight: '800', marginTop: '1rem' }}
      >
        {usermember.name === '' ? '會員' : usermember.name}
      </Link>
              ...
              return (
                <Accordion key={index} className='no-arrow-accordion'>
                  <Accordion.Item eventKey={`${index}`}>
                    ...
                    >
                      <span style={{ fontSize: '32px', margin: 0 }}>
                        {item.text}
                      </span>
                    </Accordion.Header>
                    ...
                    <Accordion.Body style={{ backgroundColor: '#D0D0D0' }}>
                      {index === 1 ? (
                        <div key={index} className={`${styles.linksContainer}`}>
                          <Link to='/switch' className='nav-link-no-arrow'>
                            資料維護
                          </Link>
                          {!isGoogle ? (
                            <>
                              <br />
                              <Link to='/fix' className='nav-link-no-arrow'>
                                修改密碼
                              </Link>
                            </>
                          ) : (
                        ...
                        <div key={index} className={`${styles.linksContainer}`}>
                          <Link to='/commit' className='nav-link-no-arrow'>
                            發案紀錄
                          </Link>
                          <br />
                          <Link to='/service' className='nav-link-no-arrow'>
                            接案紀錄
                          </Link>
                        </div>
                      )}
