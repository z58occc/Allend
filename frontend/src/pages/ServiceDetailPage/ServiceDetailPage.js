        ...
        <div className='row mt-5 p-5'>
          ...
          <div className='col-sm-9'>
            {serve.service && serve.service.length > 0 && (
              <div
                style={{
                  ...
                  boxShadow: '0 0 1px 2px skyblue',
                }}
              >
                <div style={{ width: '440px', height: '300px' }}>
                  <img
                    src={`data:image/jpeg;base64,${serve.service[0].image}`}
                    alt=''
                ...
                >
                  <div style={{ height: '35px' }}>
                    服務名稱：{serve.service && serve.service[0].s_name}
                  </div>
                  <div style={{ height: '35px' }}>
                    服務報價：{serve.service && serve.service[0].s_amount}
                    &nbsp;/&nbsp;{serve.service && serve.service[0].s_unit}
                  </div>
                  <div style={{ height: '35px' }}>
                    評價：
                    {serve.avg_star !== 0
                      ? Array.from({ length: serve.avg_star }, (_, i) => (
                          <CiStar key={i} />
                        ))
                      : '尚無評價'}
                  </div>
                  <div style={{ height: '35px' }}>
                    服務地點：{serve.service && serve.service[0].country_city}
                    <br></br>
                  </div>
                  <div
                    onClick={isLoggedIn ? () => toggleChat(mid) : handleShow}
            ...
            >
              ...
              >
                {serve.service_comments &&
                  serve.service_comments.map((item, index) => (
            ...
            <div className='row mt-4'>
              <div style={{ fontSize: '1.2rem', padding: '0 0 1.5rem 10px' }}>
                其他服務
              </div>
              {serve.other_serve &&
                serve.other_serve.map((item, index) => (
                  <Link
                    key={index}
